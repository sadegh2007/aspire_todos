...
export const TodoListItem = ({todo, onRemoved, onUpdated}: TodoListItemProps) => {
    ...
    return <tr>
        <td className='flex items-center'>
            ...
            {
                inEditMode ? <></> : <DefaultActionButtons loading={loading} onEdit={() => setInEditMode(true)} onRemove={onRemove} />
            }
        </td>
    </tr>
}

const DefaultActionButtons = ({ loading, onRemove, onEdit }: { loading: boolean, onRemove: () => Promise<void>, onEdit: () => void }) => {
    return (
        <div className="flex gap-1">
            <button onClick={onRemove} disabled={loading} className='focus:outline-0 btn btn-square btn-sm btn-error'>
                {
